...
#                                       random_state=455,  # For reproducibility
#                                       stratify=df_train['Neighborhood'])  # Maintain proportions
# %%
print("\nTraining Set:\n", train_set)
print("\nTest Set:\n", test_set)
...
class df_scaler(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        if self.method == 'standard':
            ...
            self.scl.fit(X) # fit will learnt the parameters needed to scale the data
    ...
    def transform(self, X):
        ...
        columns_in_X = set(X.columns)
        columns_in_scl = set(self.scl.feature_names_in_)

        # Features in X but not in scl
        missing_in_scl = columns_in_X - columns_in_scl
        # Features in scl but not in X
        missing_in_X = columns_in_scl - columns_in_X

        #print("Features in X but missing in scl:", missing_in_scl)
        #print("Features in scl but missing in X:", missing_in_X)

        #print(list(X.columns))  # Print column order in X
        #print(list(self.scl.feature_names_in_))  # Print column order in scl
        X = X.loc[:, self.scl.feature_names_in_] # to order the columns in the same way
...
# %% Let's test the dummify class giving as input a categorical feature
...
""" Ideally we want out pipeline to be this way:
- Clean the data
...
class tr_numeric(BaseEstimator, TransformerMixin):
    def __init__(self, SF_room=True): # standard value is True -> it will create a new feature
        self.columns = []  # useful to well behave with FeatureUnion
    ...
    def SF_per_room(self, X): # if applying standard transformations it would have been impossible to create a new feature in this way
        if self.SF_room:
...
# So the pipeline for numeric features will be

#numeric_pipe = Pipeline([('fs', feat_sel(dtype='numeric')),# select only the numeric values
#                         ('imputer', df_imputer(strategy='median')), # impute the missing values with the median of each column
#                         ('transf', tr_numeric(SF_room=True)), # remove the skew and create a new feature
#                         ('scl', df_scaler(method='standard'))]) # scale the data
#
#full_pipe = Pipeline([('gen_cl', general_cleaner()),
#                      ('num_pipe', numeric_pipe)]) # put the cleaner on top because we like it clean
#
#tmp = train_set.copy()
